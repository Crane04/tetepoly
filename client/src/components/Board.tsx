...
function SpaceCell({
  ...
  const nameFontSize = isCorner
    ? "clamp(8px,1.8cqw,18px)"
    : "clamp(5px,1.1cqw,12px)";
  const iconFontSize = isCorner
    ? "clamp(14px,3cqw,36px)"
    : "clamp(9px,1.8cqw,20px)";
    ...
    >
      ...
      >
        {propColor ? (
          ...
          >
            {ownerName && !space.hasHotel && space.houses === 0 && (
              <span
                style={{
                  fontSize: "clamp(4px,0.9cqw,9px)",
                  fontWeight: 700,
                  color: ownerColor ?? "#fff",
              ...
              >
                {ownerName.slice(0, 5).toUpperCase()}
              </span>
            )}
            {space.hasHotel ? (
              <div
...
export default function Board({
  ...
  function ownerName(space: BoardSpace): string | null {
    if (!space.ownerId) return null;
    return players.find((p) => p.id === space.ownerId)?.name ?? null;
  }
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: "6%", width: "60%" }}>
          ...
          >
            <span
              style={{ fontSize: "clamp(8px,1.8cqw,20px)", color: "#253545" }}
            >
              ⟐
            </span>
            <span
