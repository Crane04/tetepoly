...
const ROOM_META: Record<
  string,
  { tag: string; gradient: string; tagColor: string }
> = {
  "room-1": {
    tag: "BEGINNER",
    ...
    <div className="min-h-screen bg-[#0a0c10] text-slate-100 flex flex-col">
      ...
      <header className="border-b border-[#1a1f2c] px-6 h-14 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold tracking-[0.18em] text-xs uppercase text-slate-300">
            NOPOLY
          </span>
        </div>
        <div
          className={`flex items-center gap-1.5 text-xs tracking-[0.15em] uppercase ${connected ? "text-teal-600" : "text-red-600"}`}
        >
          <div
            className={`w-1.5 h-1.5 rounded-full ${connected ? "bg-teal-600" : "bg-red-600"}`}
          />
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full px-6 py-10">
        {/* Section heading */}
        <div className="mb-10">
          <p className="text-[10px] text-[#2a3848] tracking-[0.25em] uppercase mb-2">
            — High-Stakes Browser
          </p>
          <h1 className="text-4xl font-bold tracking-tight uppercase text-slate-100">
            The Rooms
          </h1>
        </div>
        ...
        {/* Name input */}
        <div className="mb-10">
          <label className="block text-[10px] text-[#3a4a5a] tracking-[0.2em] uppercase mb-2">
            Your Name
          </label>
          <input
          ...
          />
          {nameError && (
            <p className="text-red-500 text-xs mt-1.5 font-mono">{nameError}</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {availableRooms.length === 0 && (
            <p className="col-span-2 text-[#2a3848] text-xs tracking-[0.2em] uppercase font-mono">
              Syncing rooms…
            </p>
          )}
      ...
      >
        ...
        <div className="relative z-10 flex items-start justify-between">
          ...
          <div className="text-right">
            ...
            {room.waitingCount > 0 && (
              <p className="font-mono text-[10px] text-[#2a6a4a]">
                {room.waitingCount} waiting
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-end justify-between">
          <div>
            <p className="font-mono text-base font-semibold text-slate-300">
              ${room.startingMoney.toLocaleString()}{" "}
              <span className="text-[#3a4a5a] text-xs font-normal tracking-widest">
                START
              </span>
            </p>
