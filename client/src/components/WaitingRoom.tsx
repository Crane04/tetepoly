      ...
      <header className="border-b border-[#1a1f2c] px-6 h-14 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-[10px] text-[#2a3848] tracking-[0.2em] uppercase">
              Room
            </p>
            <p className="font-mono text-xs text-[#3a4a5a] mt-0.5">
              {currentGameId}
            </p>
          </div>
          <button
      ...
      <div className="flex-1 flex flex-col md:flex-row relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-wrap justify-center gap-4">
            {lobbyPlayers.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs font-semibold text-slate-300 text-center truncate w-full">
                    {p.name}
                  </p>
            ...
            {Array.from({ length: emptySlots }).map((_, i) => (
              ...
              >
                ...
                <span className="text-[10px] text-[#1e2530] tracking-[0.15em] uppercase">
                  WAITING
                </span>
              </div>
            ))}
            ...
            {lobbyPlayers.length < 2 ? (
              ...
              <>
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[10px] text-amber-600 tracking-[0.2em] uppercase">
                    Game starting in
                  </p>
                </div>
                <p className="font-mono text-5xl font-bold text-amber-500 tabular-nums">
                  {countdown}
                </p>
              </>
            ) : (
        ...
        <div className="md:absolute md:bottom-6 md:left-6 w-full md:w-72 bg-[#0c0e15] border border-[#1a1f2c] rounded-lg overflow-hidden shrink-0">
          <div className="flex items-center gap-2 px-4 py-2.5 border-b border-[#1a1f2c]">
            ...
            <span className="text-[10px] tracking-[0.2em] uppercase text-[#3a4a5a]">
              Comms Link
            </span>
          </div>
          <div className="px-4 py-3 flex flex-col gap-2 max-h-36 overflow-y-auto">
            {commsLog.map((entry, i) => (
              <p
                key={i}
                className="text-xs text-[#3a4a5a] leading-relaxed font-mono"
              >
